<div class="row mb-3" id="cashier-stats-widget">
    <div class="col-md-3 mb-3 mb-md-0">
        <div class="card bg-primary text-white mb-0">
            <div class="card-header">
                <h4 class="mb-0">{{ __('cashier.stats.current_balance') }}</h4>
            </div>
            <div class="card-body">
                <h2 class="mb-0" id="stats-cashier-current">{{ number_format($widget['stats']['current_balance'], 2) }} ₾</h2>
            </div>
        </div>
    </div>
    <div class="col-md-3 mb-3 mb-md-0">
        <div class="card bg-secondary text-white mb-0">
            <div class="card-header">
                <h4 class="mb-0">{{ __('cashier.stats.opening') }}</h4>
            </div>
            <div class="card-body">
                <h2 class="mb-0" id="stats-cashier-opening">{{ number_format($widget['stats']['opening_balance'], 2) }} ₾</h2>
            </div>
        </div>
    </div>
    <div class="col-md-3 mb-3 mb-md-0">
        <div class="card bg-success text-white mb-0">
            <div class="card-header">
                <h4 class="mb-0">{{ __('cashier.stats.cash_in') }}</h4>
            </div>
            <div class="card-body">
                <h2 class="mb-0" id="stats-cashier-in">{{ number_format($widget['stats']['cash_in'], 2) }} ₾</h2>
            </div>
        </div>
    </div>
    <div class="col-md-3">
        <div class="card bg-danger text-white mb-0">
            <div class="card-header">
                <h4 class="mb-0">{{ __('cashier.stats.cash_out') }}</h4>
            </div>
            <div class="card-body">
                <h2 class="mb-0" id="stats-cashier-out">{{ number_format($widget['stats']['cash_out'], 2) }} ₾</h2>
            </div>
        </div>
    </div>
</div>
