<div class="col-12">
    <div class="card">
        <div class="card-header">
            <div>
                <h3 class="card-title mb-0">{{ __('dashboard.payments.title') }}</h3>
                <div class="text-muted small">{{ __('dashboard.payments.subtitle') }}</div>
            </div>
            <div class="d-flex flex-wrap align-items-end justify-content-between gap-2 mt-2">
                <div class="d-flex flex-wrap align-items-end gap-2">
                    <div>
                        <label class="form-label small mb-1" for="daily-payments-from">{{ __('dashboard.chart.from') }}</label>
                        <input type="date" id="daily-payments-from" class="form-control form-control-sm">
                    </div>
                    <div>
                        <label class="form-label small mb-1" for="daily-payments-to">{{ __('dashboard.chart.to') }}</label>
                        <input type="date" id="daily-payments-to" class="form-control form-control-sm">
                    </div>
                    <button type="button" class="btn btn-sm btn-primary" id="daily-payments-apply">{{ __('dashboard.chart.apply') }}</button>
                    <button type="button" class="btn btn-sm btn-outline-secondary" id="daily-payments-reset">{{ __('dashboard.chart.reset_range') }}</button>
                </div>
        ...
        <div class="card-body">
            <div class="row g-3 mb-2" id="daily-payments-summary-totals">
                <div class="col-6 col-md-2">
                    <div class="text-muted small">{{ __('dashboard.chart.total') }}</div>
                    <div class="h3 mb-0" id="daily-payments-total-amount">—</div>
                </div>
                <div class="col-6 col-md-2">
                    <div class="text-muted small">{{ __('dashboard.payments.count') }}</div>
...
<script>
(function () {
    var i18n = @json(__('dashboard.payments'));
    var i18nChart = @json(__('dashboard.chart'));
    ...
    function renderChart(data) {
        ...
        var methodLabels = data.methodLabels || {};
        var datasets = [];
        ...
        types.forEach(function (type) {
            methods.forEach(function (method, index) {
                ...
                datasets.push({
                    label: (methodLabels[method] || method) + ' · ' + (typeLabels[type] || type),
            ...
            options: {
                    ...
                    tooltip: {
                        callbacks: {
                            beforeBody: function (tooltipItems) {
                                ...
                                var lines = [i18nChart.total + ': ' + formatMoney(total)];
                ...
                scales: {
                    ...
                    y: {
                        stacked: true,
                        beginAtZero: true,
                        title: { display: true, text: i18n.axis },
                        ticks: {
